...
class CustomDataset(Dataset):
    def __init__(self, csv_path, lookup, idx, use_cache=False, split=True, test=False, split_start=None,
                 split_end=None):
        ...
        if use_cache:
            ...
            # splitting
            if split:
                self.train["x_frame"] = self.dataframe.drop(['TYPE'], axis=1).drop(range(split_start, split_end))
                self.train["y_frame"] = self.dataframe["TYPE"].copy().drop(range(split_start, split_end))

                self.test["x_frame"] = self.dataframe.drop(['TYPE'], axis=1).iloc[split_start:split_end]
                self.test["y_frame"] = self.dataframe["TYPE"].iloc[split_start:split_end]
            else:
